...
• Similar to SJF but with preemption
• Whenever an arrival happens or a process finishes, rechecks for the shortest time to finish a process and starts based on that
• lots of overhead for switching processes
...
4. Compare algorithm behaviors and outcomes";
        ...
        /// Shortest Remaining Time First - selects process with minimum burst time, recheck at every arrival and every process finish.
        /// </summary>
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            
            var results = new List<SchedulingResult>();
            var currentTime = 0; // time progressed

            var remainingTimes = new Dictionary<string, int>(); // burst time value of each process at given point
            var startTimes = new Dictionary<string,int>(); // -1 for not started, is the value of current time at arrival
            
            var processMap = new Dictionary<string, ProcessData>();
            ...
            var completed = new HashSet<string>(); // tracks completed processes

            while( completed.Count < processes.Count)
            {

                // make a list of every arrived process that is not yet completed
                var available = new List<ProcessData>();
                foreach (var x in processes)
                {
                    if(x.ArrivalTime <= currentTime && !completed.Contains(x.ProcessID))
                    {
                        available.Add(x);
                    }
                }

                // jumps to the earliest arrival
                if(available.Count == 0)
                {
                    foreach(var y in processes)
                    {
                        if (!completed.Contains(y.ProcessID))
                        {
                            currentTime = processes.Min(y => y.ArrivalTime);
                        }
                    }
                }

                //sort list of processes first by remaining times, with arrival time as a tie breaker.
                // orderby and thenby are these weird enumarble methods that i found that make this much simpler then writing out a whole thing for sorting a list of arrays
                var current = available.OrderBy(p => remainingTimes[p.ProcessID]).ThenBy(p => p.ArrivalTime).First();

                if(startTimes[current.ProcessID] == -1)
                {
                    startTimes[current.ProcessID] = currentTime;
                }
                
                var nextArrival = int.MaxValue;
                foreach (var z in processes)
                {
                    //if arrival time is greater(later) then current time, and process isnt complete
                    if(z.ArrivalTime > currentTime && !completed.Contains(z.ProcessID)) 
                    {
                        //if arrival time is less then current next arrival (Finding Min)
                        if(z.ArrivalTime < nextArrival)
                        {
                            nextArrival = z.ArrivalTime;
                        }
                    }
                }

                // determine which comes first, the next arrival or the end of the current process
                var runUntil = Math.Min(currentTime + remainingTimes[current.ProcessID], nextArrival);

                //time till next interupt
                var runTime = runUntil - currentTime;

                // burst time
                remainingTimes[current.ProcessID] -= runTime;
                ...
                if (remainingTimes[current.ProcessID] == 0)
                {
                    ...
                    var proc = processMap[current.ProcessID];
                    var finishTime = currentTime;
                    var turnAroundTime = finishTime - proc.ArrivalTime;
                    var waitingTime = turnAroundTime - proc.BurstTime;

                    results.Add(new SchedulingResult{
                        ProcessID = proc.ProcessID,
                        ArrivalTime = proc.ArrivalTime,
                        BurstTime = proc.BurstTime,
                        StartTime = startTimes[proc.ProcessID],
                        FinishTime = finishTime,
                        WaitingTime = waitingTime,
                        TurnaroundTime = turnAroundTime
                    });
        ...
        private List<SchedulingResult> RunPriorityAlgorithm(List<ProcessData> processes)
        {
            ...
            while (remainingProcesses.Count > 0)
            {
                ...
                var nextProcess = availableProcesses.OrderBy(p => p.Priority).ThenBy(p => p.ArrivalTime).First();
        ...
        /// <summary>
        /// STUDENTS: CFS algorithm implmentation using DataGrid data
        /// Completely Fair Scheduling- processes accumulate "virtual runtime" as they use the CPU.
        /// the process with the lowest "virtual runtime"(vruntime) is the next process to be chosen
        /// vruntime grows inversly proportionate to priority. So high prio = slower vruntime growth and vice versa
        /// </summary>
        private List<SchedulingResult> RunCFSAlgorithm(List<ProcessData> processes)
        {
            var results = new List<SchedulingResult>();
            var currentTime = 0;

            var vruntimeMap = new Dictionary<string, double>(); // accumulated "virtual runtime"
            var remainingTimes = new Dictionary<string, int>(); // burst time
            var startTimes = new Dictionary<string, int>(); // first CPU interaction
            var processMap = new Dictionary<string, ProcessData>();

            var enqueued = new HashSet<string>(); // tracks which processes have started accruing vruntime / tracks arrival essentially

            const int targetLatency = 10; // used for tuning.
            const int minGranularity = 1; // minimum slice — prevents zero-length runs
            const double BASE_WEIGHT = 10.0;

            foreach (var p in processes)
            {
                vruntimeMap[p.ProcessID]    = 0.0;
                remainingTimes[p.ProcessID] = p.BurstTime;
                startTimes[p.ProcessID]     = -1;
                processMap[p.ProcessID]     = p;
            }
            ...
            while (completed.Count < processes.Count)
            {
                ...
                var available = new List<ProcessData>(); // list of "available" processes
                foreach (var x in processes)
                {
                    if (x.ArrivalTime <= currentTime && !completed.Contains(x.ProcessID))
                        available.Add(x);
                ...
                if (available.Count == 0)
                {
                    ...
                    foreach (var x in processes)
                    {
                        if (!completed.Contains(x.ProcessID) && x.ArrivalTime < nextArrival)
                            nextArrival = x.ArrivalTime;
                    }
                    currentTime = nextArrival;
                }
 
                //To maintain fairness we have to create a "floor"
                //The floor is the minimum vruntime among already enqueued processes
                //we give newly queued processes this floor as their initial vruntime to prevent new processes from monopoloizing the CPU
                double currentMinVruntime = double.MaxValue;
                foreach (var a in available)
                {
                    if (enqueued.Contains(a.ProcessID) && vruntimeMap[a.ProcessID] < currentMinVruntime)
                        currentMinVruntime = vruntimeMap[a.ProcessID];
                }
 
                foreach (var a in available)
                {
                    if (!enqueued.Contains(a.ProcessID))
                    {
                        if (currentMinVruntime < double.MaxValue)
                        {
                            vruntimeMap[a.ProcessID] = currentMinVruntime;
                        }
                        enqueued.Add(a.ProcessID);
                    }
                }
 
                //Process selection, choose based on lowest vruntime, using arrival time as tiebreaker
                ProcessData current = null;
                double lowestVruntime = double.MaxValue;
                foreach (var a in available)
                {
                    if (vruntimeMap[a.ProcessID] < lowestVruntime || (vruntimeMap[a.ProcessID] == lowestVruntime && (current == null || a.ArrivalTime < current.ArrivalTime)))
                    {
                        lowestVruntime = vruntimeMap[a.ProcessID];
                        current = a;
                    }
                }
 
                //Houston we have touchdown (on the CPU)
                if (startTimes[current.ProcessID] == -1)
                    startTimes[current.ProcessID] = currentTime;
 
                //Time slice math, each process gets a portion of the targetLatency based on weight.
                int totalWeight = 0;
                foreach (var a in available)
                    totalWeight += Math.Max(1, a.Priority); // prevent errors from 0 priority null division
 
                int weight = Math.Max(1, current.Priority);
                int slice  = Math.Max(minGranularity, (int)Math.Round((double)weight / totalWeight * targetLatency));
 
                //Similar to SRTF it pauses to choose which process to run upon every arrival and process completion
                var nextArrivalTime = int.MaxValue;
                foreach (var z in processes)
                {
                    if (z.ArrivalTime > currentTime && !completed.Contains(z.ProcessID) && z.ArrivalTime < nextArrivalTime)
                    {
                        nextArrivalTime = z.ArrivalTime;
                    }
                }
 
                int runTime = Math.Min(slice, remainingTimes[current.ProcessID]);
                int timeUntilNextArrival = nextArrivalTime - currentTime;
                if (timeUntilNextArrival > 0 && timeUntilNextArrival < runTime)
                {
                    runTime = timeUntilNextArrival;
                }
                if (runTime <= 0) runTime = 1; // stall countermeasure
 
                // moves to next arrival or process finish
                remainingTimes[current.ProcessID] -= runTime;
                currentTime += runTime;
 
                // Vruntime math
                // each second of "real" time is equal to (BASE_WEIGHT / weight) in vruntime time.
                // non "BASE_WEIGHT" weight determined by priority
                vruntimeMap[current.ProcessID] += runTime * (BASE_WEIGHT / weight);
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            listView1.Columns.Add("Response Time", 90, HorizontalAlignment.Center);
            ...
            foreach (var result in results)
            {
                ...
                item.SubItems.Add((result.StartTime - result.ArrivalTime).ToString());
            ...
            var CPUUtil = ((double)TotalBurst / results.Max(result => result.FinishTime)) * 100;
            Throughput = ((double)results.Count / results.Max(result => result.FinishTime));
            ...
            summaryItem.SubItems.Add($"Avg Response: {avgResponse:F2}");
            summaryItem.SubItems.Add($"CPU Utilization %: {CPUUtil:F2}");
            summaryItem.SubItems.Add($"Throughput: {Throughput:F4}");
        ...
        /// <summary>
        /// Exports the current scheduling results displayed in listView1 to a CSV file.
        /// </summary>
        private void ExportResults_Click(object sender, EventArgs e)
        {
            if (listView1.Items.Count == 0 || listView1.Columns.Count == 0)
            {
                MessageBox.Show("No results to export. Run one of the algorithms first.",
            ...
            using (var saveDialog = new SaveFileDialog())
            {
                ...
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            // Build the header row by looping through columns
                            string headerLine = "";
                            for (int i = 0; i < listView1.Columns.Count; i++)
                            {
                                headerLine += listView1.Columns[i].Text;
                                if (i < listView1.Columns.Count - 1)
                                    headerLine += ",";
                            }
                            writer.WriteLine(headerLine);

                            // Write each row — item.Text is the first column, SubItems has all columns
                            // SubItems[0] is the same as item.Text so we skip it to avoid duplicates
                            foreach (ListViewItem item in listView1.Items)
                            {
                                string rowLine = item.Text;
                                for (int i = 1; i < item.SubItems.Count; i++)
                                {
                                    rowLine += "," + item.SubItems[i].Text;
                                }
        ...
        private void ApplyLightTheme()
        {
            ...
            btnSRTF.BackColor = Color.LemonChiffon;
            btnCFS.BackColor = Color.Ivory;
        ...
        private void SRTFButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                // Update Results tab with detailed scheduling results
                DisplaySchedulingResults(results, "SRTF - Shortest Remaining Time Left");
        ...
        /// <summary>
        /// Executes the Completely Fair Scheduling algorithm using DataGrid data.
        /// STUDENTS: This demonstrates how to use GetProcessDataFromGrid() instead of prompts
        /// Use this pattern for your custom algorithm implementations
        /// </summary>
        private void CFSButton_Click(object sender, EventArgs e)
        {
            var processData = GetProcessDataFromGrid();
            if (processData.Count > 0)
            {
                // STUDENTS: Example implementation using DataGrid data
                var results = RunCFSAlgorithm(processData);
 
                // Update Results tab with detailed scheduling results
                DisplaySchedulingResults(results, "CFS - Completely Fair Scheduler");
